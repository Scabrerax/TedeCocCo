const rules = {
  ...
  "m": (max) => ({ max:{value: Number.parseInt(max), message: `*El valor minimo es ${max}`} }),
  "p": (min) => ({ min:{value: Number.parseInt(min), message: `*El valor maximo es ${min}`} }),
  "u": {pattern: {value:/\b[A-Z]+\b/ , message: `*Solo se permiten mayusculas`}},
  "l": {pattern: {value:/\b[a-z]+\b/ , message: `*Solo se permiten minusculas`}},
